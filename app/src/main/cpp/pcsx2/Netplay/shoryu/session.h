...
#include "zed_net.h"
#include "../NetplayLanSettingsAndroid.h"
...
namespace shoryu
{
	...
	struct message
	{
		...
		inline void deserialize(shoryu::iarchive& a)
		{
			...
			side = cmdSide >> 5;
			// 线协议固定宽度：IPv4 地址/端口与 zed_net_address_t 字段完全一致
			// （unsigned int = 4 字节、unsigned short = 2 字节），PC 端（MSVC LLP64）
			// 历史上把 addr 写成 `unsigned long` 也是 4 字节因此能跑通；Android NDK
			// 是 LP64，`unsigned long` 为 8 字节，会越界读触发 archive overflow/abort，
			// 导致房主侧 shoryu receive_loop 崩溃。这里固定为 unsigned int 与 PC 对齐。
			unsigned int addr;
			unsigned short port;
			switch(cmd)
			{
	...
	public:
        ...
        {
            if (m_host) {
                { ::fprintf(stderr, "[shoryu] session: invalid caller\n"); ::abort(); }
            } else {
                auto pred = [&]() -> bool {
		...
		bool create(int players, const StateType& state, const state_check_handler_type& handler, int timeout = 0)
		{
			...
			m_host = true;
			m_num_players = 1;
            m_mcd_sync = g_LanNetplaySettings.MemcardSync;
		...
		inline void clear_queue()
		{
			if(_current_state == MessageType::None)
				{ ::fprintf(stderr, "[shoryu] session: invalid state\n"); ::abort(); }
		...
		inline void set(const FrameType& frame)
		{
			...
			if (m_host && (g_LanNetplaySettings.ClientOnlyDelay)) {
                destFrame += 1;
            } else {
                destFrame += _delay;
            }
		...
		inline void send_chatmessage(const std::string &message)
		{
			...
			// 与 wire 格式一致：首字节 cmd|side，供房主侧 echo / 成员解析（pcsx2-online NetplayFrameSync 同构）
			msg.side = static_cast<uint8_t>(_side & 0x07);
			queue_message(msg);
			send();
			_connection_cv.notify_all();
		}
	protected:
		...
		void join_recv_handler(const zed_net_address_t& ep, message_type& msg)
		{
			...
			log << "\n";
#endif
			// 与 pcsx2-online 逻辑等价；房主 UDP 回包源端口可能与成员 dial 的端口不一致，仅 Chat 等控制类消息需放宽，否则会丢房主聊天。
			if (ep.host != _host_ep.host)
				return;
			if (ep.port != _host_ep.port &&
				msg.cmd != MessageType::Chat && msg.cmd != MessageType::Delay && msg.cmd != MessageType::Ready &&
				msg.cmd != MessageType::Ping && msg.cmd != MessageType::MCDSync && msg.cmd != MessageType::Info)
				return;
			std::unique_lock<std::mutex> lock(_connection_mutex);
		...
		void recv_hdl(const zed_net_address_t& ep, message_type& msg)
		{
			...
			// 成员侧：房主 Chat 等可能从与 listen 不同的 UDP 源端口发出，不能只按端口过滤（与 join_recv_handler 一致）
			if (!m_host)
			{
				if (ep.host != _host_ep.host)
					return;
				if (ep.port != _host_ep.port &&
					msg.cmd != MessageType::Chat && msg.cmd != MessageType::Delay && msg.cmd != MessageType::Ready &&
					msg.cmd != MessageType::Ping && msg.cmd != MessageType::MCDSync && msg.cmd != MessageType::EndSession)
					return;
			}
