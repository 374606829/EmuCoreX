...
namespace shoryu
{
	...
	struct datagram_header
	{
		...
		inline void acknoledge(int64_t id)
		{
			if(id <= 0)
			{
				/* -fno-exceptions: cannot throw */
				::fprintf(stderr, "[shoryu] datagram_header: invalid id\n");
				::abort();
			}
			if(!ack_first_id)
			{
			...
			else
			{
				...
				if(offset >= 32)
				{
					::fprintf(stderr, "[shoryu] datagram_header: offset cannot exceed 32\n");
					::abort();
				}
				ack_mask.set(offset);
			}
		...
		{
			a.write(const_cast<char*>(PROTOCOL_ID), PROTOCOL_ID_LEN);
		...
		inline void deserialize(iarchive& a)
		{
			...
			{
				/* -fno-exceptions: silently ignore bad protocol id (stream error) */
				::fprintf(stderr, "[shoryu] datagram_header: invalid protocol id\n");
				/* do not abort here — let caller handle partial read */
			}
		}
	protected:
