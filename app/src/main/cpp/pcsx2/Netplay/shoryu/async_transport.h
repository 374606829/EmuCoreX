...
namespace shoryu
{
	...
	template<OperationType Operation, int BufferSize>
		...
		public:
			...
			transaction_data<Operation,BufferSize>& next()
			{
				...
				m_next_buffer = (i + 1) % BufferQueueSize;  /* fix deprecated volatile++ */
				return buffer[i];
			}
		protected:
	...
	{
	public:
		typedef peer<DataType> peer_type;
		typedef peer_data<DataType> peer_data_type;
		...
		inline int port()
		{
			/* Android/POSIX: getsockname to retrieve bound port */
			struct sockaddr_in addr{};
			socklen_t len = sizeof(addr);
			if (::getsockname(m_socket.handle, (struct sockaddr*)&addr, &len) == 0)
	...
	protected:
		...
		void finalize(const transaction_data<OperationType::Recv,BufferSize>& transaction)
		{
				...
				/* -fno-exceptions: call directly; overflow_check aborts on error */
